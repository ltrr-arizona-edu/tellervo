#!/usr/bin/env python3
"""Report exact tblmeasurement and tblvmeasurement counts across databases."""
...
class CountResult:
    database: str
    tblmeasurement: Optional[int]
    tblvmeasurement: Optional[int]
    status: str
    detail: str = ""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Report exact tblmeasurement and tblvmeasurement row counts "
...
def count_database(psql: Psql, database: str) -> CountResult:
    try:
        relations = scalar(
            ...
            """
SELECT concat_ws(E'\\t',
    (to_regclass('public.tblmeasurement') IS NOT NULL)::text,
    (to_regclass('public.tblvmeasurement') IS NOT NULL)::text);
""",
        ).split("\t")
        if len(relations) != 2:
            raise PsqlError(f"expected two relation checks, received {len(relations)}")

        missing = []
        if relations[0] != "true":
            missing.append("public.tblmeasurement")
        if relations[1] != "true":
            missing.append("public.tblvmeasurement")
        if missing:
            return CountResult(
                database, None, None, "MISSING", "missing " + ", ".join(missing))
        ...
        counts = scalar(
            ...
            """
SELECT concat_ws(E'\\t',
    (SELECT count(*) FROM public.tblmeasurement)::text,
    (SELECT count(*) FROM public.tblvmeasurement)::text);
""",
        ).split("\t")
        if len(counts) != 2:
...
def write_table(results: list[CountResult]) -> None:
    headers = ("DATABASE", "TBLMEASUREMENT", "TBLVMEASUREMENT", "STATUS", "DETAIL")
    rows = [
        (
            result.database,
            "-" if result.tblmeasurement is None else str(result.tblmeasurement),
            "-" if result.tblvmeasurement is None else str(result.tblvmeasurement),
            result.status,
            result.detail,
        )
