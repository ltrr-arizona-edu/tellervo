...
// TODO: decimal alignment.  for example, "{.}" should align to a decimal point, and
// "{/*}" should align to either '*' or '/'.
...
public class TabbedLineFactory {
    ...
    private List tabs; // list of BAR/LEFT/CENTER/RIGHT, or Number (meaning "skip this much")
    ...
    /** Create a tabbed-line factory, given a line specification (see above).
	...
	for (int i=0; i<n; i++) {
	    ...
	    // skip whitespace (but i need to watch for it, because it's a delimiter)
	    if (t.trim().length() == 0)
		continue;
    ...
    /**
       ...
       <p>The values should be separated by tab characters (<code>'\t'</code>).
       (Yes, that means you can't have tab characters in your text.  Why
       would you want to?  What would it mean?)</p>

       <p>If you have fewer elements in this line than your spec string expects,
       as many as you provide are used; beyond that, blanks are used.  If you
       provide too many elements, as many are expected are used (from the start
       of the string), and the rest are ignored.</p>

       @param values a string containing all of the values for this line
    */
	...
	String vals[] = Text.splitBy(values, '\t');
	for (int i=0; i<vals.length; i++) {
	    vals[i] = vals[i].trim();
	    if (vals[i].equals("")) // java bug: g.drawString("",x,y) throws illegal argument.  (dumb...)
		vals[i] = " ";
	}
	for (int i=0; i<vals.length; i++)
	    val.add(vals[i]); // is there a method for array->list?  i can't find one...
	...
	public void print(Graphics g, PageFormat pf, float y) {
		...
		if (tabs.get(i) instanceof Number) {
			...
			float col = g.getFontMetrics().stringWidth(val.get(next).toString()); // width of this column's text
