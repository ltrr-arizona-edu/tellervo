    ...
    */
	...
	lines.add(f.makeLine("\t0\t1\t2\t3\t4\t5\t6\t7\t8\t9"));
		...
		if (!s.range.contains(fake.add(i)))
		    ...
		    data += "\t" + s.data.get(fake.add(i).diff(s.range.getStart()));
	    }
	...
	// build spec
	String spec = "> 4% ";
	for (int i=0; i<10; i++) spec += "5.7% <"; // data
	spec += " 5% ";
	for (int i=0; i<10; i++) spec += "3.4% <"; // count
...

	{
		...
		for (int i=0; i<10; i++) {
			...
			vals += ((Number) s.data.get(decade.add(i).diff(s.range.getStart()))).intValue();
		}
		// count
		for (int i=0; i<10; i++) {
		    vals += "\t";
		    if (s.range.contains(decade.add(i)))
			vals += ((Number) s.count.get(decade.add(i).diff(s.range.getStart()))).intValue();
		}
	...
	int threes = s.count3SampleIntervals();
        lines.add(new TextLine("Number of intervals with >3 samples: " + threes));
    ...
    private void printElements() {
        lines.add(new TextLine("This data set is composed of the following files:"));
	...
	TabbedLineFactory f = new TabbedLineFactory("> 12% <> 58% ^ 7% ^ 8% ^ 8% ^ 7% <");

	// add table header -- skip "unmeas/pre/post" headers
	// (not enough space, and it's fairly obvious, anyway)
	lines.add(f.makeLine("ID \t \t Filename \t Pith \t \t Range \t \t Terminal")); // FIXME: i18n me!
	...
	// write out all elements
	    ...
	    // not loaded?
	    if (e.details == null) {
		String mark = ((e.error instanceof FileNotFoundException) ? footnoteSymbol1 : footnoteSymbol2);
		mark = mark + " ";
		lines.add(f.makeLine("\t " + mark + " \t " + e.filename + " \t \t \t \t "));
	    } else {
		String x = "";

		x += (e.details.containsKey("id") ? e.details.get("id").toString() : "");
		x += "\t";
		x += ""; // no footnote
		x += "\t";
		x += (e.details.containsKey("filename") ? e.details.get("filename").toString() : ""); // title?
		x += "\t";
		x += (e.details.containsKey("pith") ? "+"+e.details.get("pith").toString() : "");
		x += "\t";
		x += (e.details.containsKey("unmeas_pre") ? "+"+e.details.get("unmeas_pre").toString() : "");
		x += "\t";
		x += e.getRange().toString();
		x += "\t";
		x += (e.details.containsKey("unmeas_post") ? "+"+e.details.get("unmeas_post").toString() : "");
		x += "\t";
		x += (e.details.containsKey("terminal") ? e.details.get("terminal").toString() : "");
	...
	if (footnote || footnote2)
	    lines.add(new EmptyLine());
	if (footnote)
	    lines.add(new TextLine(footnoteSymbol1 +
				   " This file wasn't found; it was probably moved, renamed, or deleted."));
	if (footnote2)
	    lines.add(new TextLine(footnoteSymbol2 +
    ...
    private class WeiserjahreLine implements Line {
        ...
        public void print(Graphics g, PageFormat pf, float y) {
            ...
            // decade
            g2.drawString(decade.toString(),
			  (float) pf.getImageableX(), baseline);
