...
public class Browser extends JFrame {
    ...
    */
	...
	// but (1) smaller, and (2) dimmer?
        ...
        if (searchField.isEmpty()) {
            ...
            text = "Focused on " + visibleFiles.size() + " of " + files.size() + " samples";
        }
	...
	// TODO: select row of tree which corresponds to |dir| (which automatically calls setFolder())
        ...
        // labels
	label1 = new JLabel();
	label1.setFont(label1.getFont().deriveFont(label1.getFont().getSize() * 1.4f)); // (!)
	    ...
	    // REFACTOR: EXTRACT CLASS/METHOD?
			...
			// PERF: i think this might slow down drawing a little, but i'm not positive.
                        ...
                        if (!isSelected)
                            c.setBackground(row%2==0 ? ODD_ROW_COLOR : Color.white);
	    ...
	    JPanel main = Layout.borderLayout(north,
					      null, sp, null,
					      null);
            p.add(main);
        }
    ...
    private void addViewMenus(JComponent p) {
        // -- file metadata
	p.add(new FieldCheckBoxMenuItem("name", I18n.getText("browser_name"), false));
        ...
        p.add(new FieldCheckBoxMenuItem("modified", I18n.getText("browser_modified")));
	...
	if (p instanceof JMenu)
        ...
        // -- range metadata
        p.add(new FieldCheckBoxMenuItem("range", I18n.getText("browser_range")));
        p.add(new FieldCheckBoxMenuItem("start", Builder.INDENT + I18n.getText("browser_start")));
        p.add(new FieldCheckBoxMenuItem("end", Builder.INDENT + I18n.getText("browser_end")));
        p.add(new FieldCheckBoxMenuItem("length", Builder.INDENT + I18n.getText("browser_length")));
	...
	while (i.hasNext()) {
	    MetadataTemplate.Field f = (MetadataTemplate.Field) i.next();
            p.add(new FieldCheckBoxMenuItem(f.getVariable(),
					    f.getDescription()));
	}
    }
    ...
    */
	...
	// FIXME: these need to be added to the loop below...
        ...
        same.add(new JMenuItem(Builder.INDENT + I18n.getText("browser_length")));
        same.addSeparator();
	Iterator i = MetadataTemplate.getFields();
	while (i.hasNext()) {
	    MetadataTemplate.Field f = (MetadataTemplate.Field) i.next();
	    final String glue = f.getVariable();
		    ...
		    // add row i
                    ...
                    for (int ii=0; ii<visibleFiles.size(); ii++) {
                        Object test = ((Row) visibleFiles.get(ii)).getField(glue);
                        if (test == null)
			    continue;
			if (test instanceof String && ((String) test).equalsIgnoreCase(value))
	...
	addViewMenus(view);
        ...
        search.add(new SearchMenuItem("Oak", "quercus", "species", browserModel));
        search.add(new SearchMenuItem("Tucson Format", "tucson", "filetype", browserModel));
        search.add(new SearchMenuItem("Absolutely Dated", "absolut", "dating", browserModel));
        search.add(new SearchMenuItem("Indexed with a Cubic Spline", "cubic spline", "index_type", browserModel));
        search.add(new SearchMenuItem("Modified Today", "today", "modified", browserModel));
        search.add(new SearchMenuItem("Modified Yesterday Afternoon", "yesterday pm", "modified", browserModel));
    ...
    */
	...
	try {
	    ...
	    for (int i=0; i<n; i++) {
		String filename = ((Row) visibleFiles.get(selected[i])).getPath();
    ...
    private class SearchMenuItem extends JMenuItem {
        ...
        SearchMenuItem(String name, String terms, String field, AbstractTableModel model) {
            super(name);
            tableModel = model;
...
*/
    ...
    private void doSort() {
		...
		if (reverse) {
		    ...
		    return NaturalSort.compareIgnoreCase((String) f1, (String) f2);
                }
...
//        selectFirstRow(); -- XXX do we want this?
    ...
    private static final String DEFAULT_FIELDS =
	                        "name size modified range format";
