...
public class Summary {
    ...
    */
	...
	String name = StringUtils.escapeForXML(new File(e.getFilename()).getName());
	w.write("  <sample filename=\"" + name + "\" " +
    ...
    */
	...
	Writer fw;
	if (USE_COMPRESSION)
	    fw = new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(fullFilename)));
	else
	...
	while (keys.hasNext()) {
		...
		String encodedFilename = StringUtils.escapeForXML(f.getName());
		...
		File f = new File(filename);
		w.write("  <file filename=\"" + StringUtils.escapeForXML(f.getName()) + "\" " +
			"modified=\"" + mod + "\"/>");
		w.newLine();
	    }
	}
    ...
    */
	...
	if (GZIP.isCompressed(fullFilename))
	    r = new InputStreamReader(new GZIPInputStream(new FileInputStream(fullFilename)));
	else
	...
	public void startElement(String uri, String name,
		...
		// file: add to list
		items.put(atts.getValue("filename"),
			  new MyFile(Long.parseLong(atts.getValue("modified"))));
	    } else if (name.equals("sample")) {
		element = new Element(folder + File.separator + atts.getValue("filename"));
		// -- legal?  (why?)  (why not?)
		element.lastModified = Long.parseLong(atts.getValue("modified"));
    ...
    */
    ...
    /**
       ...
       or isn't a folder
    */
    public Summary(String folder, ProgressMonitor monitor)
