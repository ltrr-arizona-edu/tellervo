...
-- Structural changes
ALTER TABLE tlkptaxon ADD COLUMN IF NOT EXISTS newlabel VARCHAR;
ALTER TABLE tlkptaxon ADD COLUMN IF NOT EXISTS htmllabel VARCHAR;
...
DROP INDEX IF EXISTS uniq_colid;
...
-- Make sure new nonblank colid values are unique.
CREATE UNIQUE INDEX IF NOT EXISTS uniq_colid ON tlkptaxon (colid) WHERE colid <> '';

-- Fix element table to include new colid
DROP TRIGGER IF EXISTS update_element_rebuildmetacache ON tblelement;
ALTER TABLE tblelement ADD COLUMN IF NOT EXISTS colid VARCHAR;
