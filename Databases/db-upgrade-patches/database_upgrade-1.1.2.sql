...
CREATE VIEW vwtblsample AS
    ...
    s.state, s.knots, s.description, dc.datecertainty, s.boxid, lc.objectcode, lc.elementcode FROM (((tblsample s LEFT JOIN 
...
CREATE VIEW vwtblelement AS
    ...
    e.diameter, e.width, e.height, e.depth, e.unsupportedxml, e.objectid, e.elementtypeid, e.elementauthenticityid, e.elementshapeid, 
    auth.elementauthenticity, shape.elementshape, tbltype.elementtype, loctype.locationtype, e.altitude, e.slopeangle, e.slopeazimuth, 
    ...
    vwt.taxonrank, unit.unit AS units FROM ((((((tblelement e LEFT JOIN tlkpelementauthenticity auth 
    ON ((e.elementauthenticityid = auth.elementauthenticityid))) LEFT JOIN tlkpelementshape shape ON ((e.elementshapeid = 
