		...
		public SampleList(String title) {
			...
			JScrollPane sp = new JScrollPane(list, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				...
				public void paint(Graphics g) {
					...
					if (hilite) {
						...
						g2.drawRect(in.left + THICKNESS / 2, in.top + THICKNESS / 2, this.getWidth() - in.right - in.left - scrollbarWidth
								- THICKNESS, this.getHeight() - in.bottom - in.top - THICKNESS);
					}
				}
			};
			add(sp);
			...
			list.setCellRenderer(new DefaultListCellRenderer() {
				...
				public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
					JLabel normal = (JLabel) super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
			...
			DragGestureListener dragger = new DragGestureListener() {
				public void dragGestureRecognized(DragGestureEvent event) {
					...
					event.startDrag(DragSource.DefaultMoveDrop, // was: DefaultCopyDrop
							img, pt, t, new Tree.EmptyDragSourceListener());
				}
			};
