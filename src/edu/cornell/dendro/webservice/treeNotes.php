...
if(!($myMetaHeader->status == "Error"))
{
    ...
    if($theMode=='read')
    {
        ...
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            ...
            {
                $sql="select * from tlkptreenote where treenoteid=$theID order by treenoteid";
            }
    ...
    if($theMode=='assign')
    {
        if($myAuth->treePermission($theTreeID, "update"))   
        {
            $mySiteNote->assignToTree($theTreeID);     
        }
        else
        {
            $myMetaHeader->setMessage("103", "Permission denied on treeid=$theTreeID");
        }
    ...
    if($theMode=='unassign')
    {
        if($myAuth->treePermission($theTreeID, "update"))   
        {
            $mySiteNote->unassignToTree($theTreeID);     
        }
        else
        {
            $myMetaHeader->setMessage("103", "Permission denied on treeid=$theTreeID");
