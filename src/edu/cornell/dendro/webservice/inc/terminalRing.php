...
class terminalRing 
{
    var $id = NULL;
    var $label = NULL;
    var $parentXMLTag = "terminalRingDictionary"; 
    ...
    {
        global $dbconn;
        $sql = "select * from tlkpterminalring where label='$theLabel'";
        ...
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            ...
            else
            {
                ...
                $this->setParamsFromDB($row['terminalringid']);
            }
        }
        else
        {
    ...
    function setParamsFromDB($theID)
    {
        ...
        global $dbconn;
        $this->id=$theID;
        $sql = "select * from tlkpterminalring where terminalringid='$theID'";
        $dbconnstatus = pg_connection_status($dbconn);
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            pg_send_query($dbconn, $sql);
            ...
            if(pg_num_rows($result)==0)
            {
                ...
                $this->setErrorMessage("903", "No records match the specified terminalRing id");
                return FALSE;
            }
            else
            {
    ...
    function writeToDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                {
                    // New record
                    $sql = "insert into tlkpterminalring (label) values ('".$this->label."')";
                }
                else
                {
                    // Updating DB
                    $sql = "update tlkpterminalring set label='".$this->label."' where terminalringid=".$this->id;
                }
    ...
    function deleteFromDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                $sql = "delete from tlkpterminalring where terminalringid=".$this->id;
