...
class pith 
{
    var $id = NULL;
    var $label = NULL;
    var $parentXMLTag = "pithDictionary"; 
    ...
    {
        global $dbconn;
        $sql = "select * from tlkppith where label='$theLabel'";
        ...
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            ...
            else
            {
                ...
                $this->setParamsFromDB($row['pithid']);
            }
        }
        else
        {
    ...
    function setParamsFromDB($theID)
    {
        ...
        global $dbconn;
        $this->id=$theID;
        $sql = "select * from tlkppith where pithid='$theID'";
        $dbconnstatus = pg_connection_status($dbconn);
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            pg_send_query($dbconn, $sql);
            ...
            if(pg_num_rows($result)==0)
            {
                ...
                $this->setErrorMessage("903", "No records match the specified pith id");
                return FALSE;
            }
            else
            {
    ...
    function writeToDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                {
                    // New record
                    $sql = "insert into tlkppith (label) values ('".$this->label."')";
                }
                else
                {
                    // Updating DB
                    $sql = "update tlkppith set label='".$this->label."' where pithid=".$this->id;
                }
    ...
    function deleteFromDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                $sql = "delete from tlkppith where pithid=".$this->id;
