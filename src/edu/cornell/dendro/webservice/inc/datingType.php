...
class datingType 
{
    var $id = NULL;
    var $label = NULL;
    var $parentXMLTag = "datingTypeDictionary"; 
    ...
    {
        global $dbconn;
        $sql = "select * from tlkpdatingtype where label='$theLabel'";
        ...
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            ...
            else
            {
                ...
                $this->setParamsFromDB($row['datingtypeid']);
            }
        }
        else
        {
    ...
    function setParamsFromDB($theID)
    {
        ...
        global $dbconn;
        $this->id=$theID;
        $sql = "select * from tlkpdatingtype where datingtypeid='$theID'";
        $dbconnstatus = pg_connection_status($dbconn);
        if ($dbconnstatus ===PGSQL_CONNECTION_OK)
        {
            pg_send_query($dbconn, $sql);
            ...
            if(pg_num_rows($result)==0)
            {
                ...
                $this->setErrorMessage("903", "No records match the specified datingType id");
                return FALSE;
            }
            else
            {
    ...
    function asXML()
    {
        ...
        if (!isset($this->lastErrorCode))
        {
            ...
            $xml.= "<datingType>".$this->label."</datingType>\n";
            return $xml;
        }
        else
        {
    ...
    function writeToDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                {
                    // New record
                    $sql = "insert into tlkpdatingtype (label) values ('".$this->label."')";
                }
                else
                {
                    // Updating DB
                    $sql = "update tlkpdatingtype set label='".$this->label."' where datingtypeid=".$this->id;
                }
    ...
    function deleteFromDB()
    {
        ...
        if($this->lastErrorCode == NULL)
        {
            ...
            if ($dbconnstatus ===PGSQL_CONNECTION_OK)
            {
                ...
                $sql = "delete from tlkpdatingtype where datingtypeid=".$this->id;
