...
class element extends elementEntity implements IDBAccessor {
	 ...
	 * @param UUID $theID        	
	 * @param String $format
	 *        	(standard or summary. defaults to standard)
	 * @return unknown
	 */
	...
	function setParamsFromParamsClass($paramsClass) {
		...
		$this->setDimensions ( $paramsClass->getDimension ( 'height' ), $paramsClass->getDimension ( 'width' ), $paramsClass->getDimension ( 'depth' ) );
	 ...
	 *
	 * @param String $format
	 *        	(one of standard, comprehensive, summary or minimal. Defaults to standard)
	 * @param String $parts
	 *        	(one of all, beginning, or end. Defaults to all)
	 * @return Boolean
	 */
	...
	/**
	 * Internal function for getting the XML representation of this element.
	 * Use asXML instead.
	 *
	 * @param String $format        	
	 * @param String $parts        	
		...
		if ($this->getLastErrorCode () == NULL) {
			if (($parts == "all") || ($parts == "beginning")) {
				...
				if ($this->getShape () != NULL)
					$xml .= "<tridas:shape normalTridas=\"" . dbhelper::escapeXMLChars ( $this->getShape () ) . "\" normalId=\"" . $this->getShape ( TRUE ) . "\" />\n";
				...
				if ($this->getAuthenticity () != NULL)
					$xml .= "<tridas:authenticity>" . dbhelper::escapeXMLChars ( $this->getAuthenticity () ) . "</tridas:authenticity>\n";
				
				$xml .= $this->location->asXML ();
				
				if ($this->getProcessing () != NULL)
					$xml .= "<tridas:processing>" . dbhelper::escapeXMLChars ( $this->getProcessing () ) . "</tridas:processing>\n";
				if ($this->getMarks () != NULL)
					$xml .= "<tridas:marks>" . dbhelper::escapeXMLChars ( $this->getMarks () ) . "</tridas:marks>\n";
				if ($this->getAltitude () != NULL)
					$xml .= "<tridas:altitude>" . dbhelper::escapeXMLChars ( $this->getAltitude () ) . "</tridas:altitude>\n";
				if (($this->getSlopeAngle () != NULL) || ($this->getSlopeAzimuth ()) != NULL) {
					$xml .= "<tridas:slope>\n";
					if ($this->getSlopeAngle () != NULL)
						$xml .= "<tridas:angle>" . dbhelper::escapeXMLChars ( $this->getSlopeAngle () ) . "</tridas:angle>\n";
					if ($this->getSlopeAzimuth () != NULL)
						$xml .= "<tridas:azimuth>" . dbhelper::escapeXMLChars ( $this->getSlopeAzimuth () ) . "</tridas:azimuth>\n";
					$xml .= "</tridas:slope>\n";
				}
				if (($this->getSoilDepth () != NULL) || (dbhelper::escapeXMLChars ( $this->getSoilDescription () ) != NULL)) {
					$xml .= "<tridas:soil>\n";
					if ($this->getSoilDescription () != NULL)
						$xml .= "<tridas:description>" . dbHelper::escapeXMLChars ( $this->getSoilDescription () ) . "</tridas:description>\n";
					if ($this->getSoilDepth () != NULL)
						$xml .= "<tridas:depth>" . dbhelper::escapeXMLChars ( $this->getSoilDepth () ) . "</tridas:depth>\n";
					$xml .= "</tridas:soil>\n";
				}
				
				if ($this->getBedrockDescription () != NULL)
