	...
	function writeToDB()
	{
		...
		if($this->lastErrorCode == NULL)
		{
			...
			if ($dbconnstatus ===PGSQL_CONNECTION_OK)
			{
                ...
                if($this->id=='1')
                {
                        // Delete members of the admin group if one or more members is present
                        $hasmembers = false;
                        foreach($this->userMembers as $item)
                        {
                            if($item!="") $hasmembers = true;
                        }
                        if($hasmembers)
                        {
                        ...
                        {
                           $this->setErrorMessage("001", "There must be at least one administrator");
                           return false;
                        }
                }
                else
                {
                        // For other groups we can safely delete all memberships and re-add
