...
class tobject extends objectEntity implements IDBAccessor {
	...
	function setParamsFromDBRow($row, $format = "standard") {
		...
		global $myMetaHeader;
		if ($debugFlag === TRUE)
			$myMetaHeader->setTiming ( "Setting object parameters for objectid " . $row ['objectid'] );
		...
		$this->setParentObjectID($row['parentobjectid']);
	...
	function setParentsFromDB() {
		...
		$firebug->log("Setting parent project for object id ".$this->getID());
	
		if (($myProjectEntityArray = dbEntity::getCachedEntity ( "projectEntityArray", $this->getProjectID() )) != NULL) {
			$this->parentEntityArray = $myProjectEntityArray;
			return;
		}
		...
		$sql = "SELECT * FROM vwtblproject WHERE projectid='".$this->getProjectID()."'";
		$dbconnstatus = pg_connection_status ( $dbconn );
		if ($dbconnstatus === PGSQL_CONNECTION_OK) {
			pg_send_query ( $dbconn, $sql );
			$result = pg_get_result ( $dbconn );
			...
			if (pg_num_rows ( $result ) == 0) {
				...
				while ( $row = pg_fetch_array ( $result ) ) {
					...
					$success = $myProject->setParamsFromDBRow($row);
					if ($success === FALSE) {
						trigger_error ( $myProject->getLastErrorCode () . $myProject->getLastErrorMessage () );
					}
						
					// Add to the array of parents
					array_push ( $this->parentEntityArray, $myProject );
				}
	
				
				$this->cacheEntity ( $this->parentEntityArray, "projectEntityArray", $this->getProjectID() );
			}
		}
	}
		...
		if ($this->getLastErrorCode () == NULL) {
			if (($parts == "all") || ($parts == "beginning")) {
				...
				if ($this->getCreator () != NULL)
					$xml .= "<tridas:creator>" . dbHelper::escapeXMLChars ( $this->getCreator () ) . "</tridas:creator>";
				if ($this->getOwner () != NULL)
					$xml .= "<tridas:owner>" . dbHelper::escapeXMLChars ( $this->getOwner () ) . "</tridas:owner>";
				...
				if ($this->hasGeometry ())
					$xml .= "<tridas:genericField name=\"tellervo.mapLink\" type=\"xs:string\">" . dbHelper::escapeXMLChars ( $this->getMapLink () ) . "</tridas:genericField>\n";
				if ($this->getCode () != NULL)
					$xml .= "<tridas:genericField name=\"tellervo.objectLabCode\" type=\"xs:string\">" . $this->getCode () . "</tridas:genericField>\n";
				if ($this->getCountOfChildVMeasurements () != NULL)
