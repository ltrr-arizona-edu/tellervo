...
public class SampleLabelRoll4xPoint375StyleB extends AbstractTellervoLabelStyle {
		...
		private Font teenyfont = new Font(Font.FontFamily.HELVETICA, 4f);

	    private final static Logger log = LoggerFactory.getLogger(SampleLabelRoll4xPoint375StyleB.class);

	
	public SampleLabelRoll4xPoint375StyleB() {
		super("Sample Label 4 x 0.375\"", "Barcode labels for samples printed on a roll of 4 x 0.375\" lables. Designed to fit on core mounts", ItemType.SAMPLE);
	}
	...
	public void outputPDFToStream(java.io.OutputStream output, ArrayList items) throws Exception {
		try {
		    ...
		    for(Object item : items)
	        {
				...
				labelText = labcodeField.getValue();
	        	...
	        	ct.addText(new Phrase(labelText, tinyfont));
	        	ct.go();
	        	...
	        	ct.setSimpleColumn(215,2 ,this.document.getPageSize().getWidth() - margin - margin , this.document.getPageSize().getHeight() - margin , 5f, Element.ALIGN_LEFT);
	        	ct.addText(new Phrase("The Site Name\nPI: Peter Brewer\nThe Name of the Project", teenyfont));
	        	ct.go();
