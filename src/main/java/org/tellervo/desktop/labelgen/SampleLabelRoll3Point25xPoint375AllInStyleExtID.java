...
public class SampleLabelRoll3Point25xPoint375AllInStyleExtID extends AbstractTellervoLabelStyle {

		private boolean useExternalID = true;		
	    ...
	    private final static Logger log = LoggerFactory.getLogger(SampleLabelRoll3Point25xPoint375AllInStyleExtID.class);

	
	public SampleLabelRoll3Point25xPoint375AllInStyleExtID() {
		super("Sample Label - All-in-one with External IDs - 3.25 x 0.375\"", "Barcode labels for samples printed on a roll of 3.25 x 0.375\" label s. Designed to fit on core mounts and includes barcode, site name, project name, PI, and external IDs.", ItemType.SAMPLE);
	...
	public void outputPDFToStream(java.io.OutputStream output, ArrayList items) throws Exception {
		try {
		    ...
		    for(Object item : items)
	        {
				...
				{
					// Use ExternalID if available.  Otherwise fall back to the labcode
					String extid = TridasUtils.getGenericFieldValueByName(s, "tellervo.externalID");	
					if(extid!=null)
					{
						sampleLabelText = extid;
					}
	        	}
